...
#             if predicted_label != "NormalVideos" and predicted_prob > 0.8:
#                 text = str(predicted_label)
#                 cv2.imwrite(f"{predicted_label}.jpg", frame)
#                 break
#             else:
#                 continue
#             break
    ...
    if count % (fps * 2) == 0:
        ...
        for i in range(4):
            ...
            if predicted_label != "NormalVideos" and predicted_prob > 0.3:
                text = str(predicted_label)
                print(f"inside loop: {predicted_label}")
                cv2.imwrite(f"{predicted_label}.jpg", frame)
                url = 'http://127.0.0.1:8000/api/images/'
                files = {'image': open(f'{predicted_label}.jpg', 'rb')}
